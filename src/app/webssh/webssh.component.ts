...
export class WebsshComponent implements OnInit {
  ...
  constructor(
    ...
    this.form.patchValue(JSON.parse(<string>window.localStorage.getItem("sshConnection")));
  }

  ngOnInit(): void {
    let sshURL = this.constantService.WEB_SSH_ENDPOINT;
    ...
    function getWebsshID() {
      ...
      try {
        if ((
          ...
          promise
            .then((response: any) => {
              ...
              if (response.id) {
                ...
                toastr.warning('credentials Invalid. ❌');
              }
            })
            .catch((error) => {
              console.log(error);
          ...
          toastr.warning('Plz Fill Necessary Fields. 🖊');
        }
      } catch (e) {
        console.log(e);
      }
    ...
    function connectWS(msg: any) {
      ...
      setTimeout(async () => {
        ...
        function current_geometry(term: any) {
          ...
          var cols: any =
            parseInt((window.innerWidth / style.width).toString(), 10) - 1;
          var rows: any = parseInt(
            (window.innerHeight / style.height).toString(),
            10
          );
        ...
        function read_as_text_with_decoder(
          file: any,
          callback: any,
          decoder: any
        ) {
        ...
        function read_as_text_with_encoding(
          file: any,
          callback: any,
          encoding: any
        ) {
        ...
        sock.onopen = function () {
          toastr.success('Terminal Connected. 🖥');
          term.open(terminal);
          term.writeln(formValue.password);
  ...
  //         axios
  //           .post(
  //             this.constantService.get_webssh_url(
  //               this.constantService.WEB_SSH_ENDPOINT
  //             ),
  //             formValue,
  //             {
  //               headers: headers,
  ...
  //     let ws_url = this.constantService.WEB_SSH_ENDPOINT.split(
  //         /\?|#/,
  //         1
  //       )[0].replace('http', 'ws'),
  //       join = ws_url[ws_url.length - 1] === '/' ? '' : '/',
